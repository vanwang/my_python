...
def remove_duplicates(excel_file):
    ...
    def select_row(group):
        # 检查是否有职位信息不为空的行
        non_empty = group[group['职位ID'].notna()]
        if len(non_empty) > 0:
            # 如果有不为空的行，返回第一个不为空的行
    ...
    # 应用去重逻辑
    result = df.groupby('职位ID', as_index=False).apply(select_row)
...
# 使用示例
excel_file = '/Users/Van/Desktop/抓取/前程无忧/数据处理/jobs.xlsx'  # 替换为你的Excel文件名
